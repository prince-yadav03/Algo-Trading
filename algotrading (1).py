...
class ShortSellBuy():
    ...
    def feed(self, market):
        if market.datas[0].high[0] > market.datas[0].high[-1]:
            ...
            return ShortSellSell(self.high)
        else:
            return self
...
class ShortSellSell():
    ...
    def feed(self, market):
        if market.datas[0].close[0] > market.ema[0]:
            return ShortSellBuy(self.high)
        elif market.datas[0].high[0] > self.high:
            market.order = market.buy()
            return Initial()
        else:
            return self
...
class ShortSellPreSell():
    ...
    def feed(self, market):
        if market.datas[0].low[0] < market.datas[0].low[-1]:
           ...
           return ShortSellSell(market.datas[0].high[-1])
...
class Buy():
    ...
    def feed(self, market):
        ...
        if self.low > market.datas[0].low[0]:
            ...
            market.order = market.sell()
            return Initial()
        elif market.datas[0].close[0] < market.ema[0]:
    ...
    def feed(self, market):
        ...
        if market.indicator.lines.p[0] > market.datas[0].close[0]:
            if market.adx > 25.6 and market.diMinus > market.adx:
                # print("short sell", market.datas[0].low[0], market.datas[0].open[0], market.datas[0].high[0], market.datas[0].close[0])
...
class TestStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        self.indicator = bt.indicators.FibonacciPivotPoint(self.data1, plot = False)
        self.state = Initial()
...
for i, t in enumerate(file['Ticker'].iloc[:400]):
    print("Current :", i+1)
    dataframe = get_historical_data(t)
